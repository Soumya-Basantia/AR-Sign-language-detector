...
class LetterBuffer:
    ...
    def add_space(self) -> bool:
        """
        Add a space to separate words in the buffer.
        Returns True if space was added.
        """
        if len(self._letters) >= self._max_letters:
            return False
        self._letters.append(" ")  # Add space
    ...
    def flush_on_space(self) -> list[str]:
        """Flush the buffer and return list of words separated by spaces."""
        if not self._letters:
            return []
        
        # Join letters, split on spaces, correct each word
        raw_text = "".join(self._letters)
        word_parts = raw_text.split()
        corrected_words = []
        for part in word_parts:
            if part:  # Skip empty
                corrected = auto_correct(part) if len(part) > 1 else part
    ...
    def display_string(self) -> str:
        """Letters with spaces for AR display, e.g. 'H E  P' (space shown as gap)."""
        return "".join(self._letters).replace(" ", "   ")
