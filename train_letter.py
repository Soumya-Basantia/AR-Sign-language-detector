...
Usage:
    python train_letter.py                    # uses MLP
    python train_letter.py --model lstm       # uses LSTM
"""
...
DATA_DIR  = "data_letters/sequences"
MODEL_DIR = "model"
...
def load_dataset(flatten=True):
    ...
    Args:
        flatten: If True, each (40, 124) sequence → 4960-dim vector.
                 If False, keep shape (40, 124) for LSTM.
...
def main():
    ...
    if args.model == "mlp":
        ...
        joblib.dump(model, os.path.join(MODEL_DIR, "model_letter.pkl"))
        print(f"\n[SAVED] model/model_letter.pkl")
    ...
    joblib.dump(le, os.path.join(MODEL_DIR, "label_encoder_letter.pkl"))
    meta = {
    ...
    with open(os.path.join(MODEL_DIR, "metadata_letter.json"), "w") as f:
        json.dump(meta, f, indent=2)

    print(f"[SAVED] model/label_encoder_letter.pkl")
    print(f"[SAVED] model/metadata_letter.json")
    print("\n✅ Training complete!")
